...
constexpr int maxn = 116+10; // remember to calculate. if tle, check maxn first.
/*
    ...
    对吗？
*/
int vis[maxn],a[maxn], prime[maxn];
int32_t main()
{
    int _ = 0;
    int n;
    ...
    while(cin >> n){
        ...
        function<void (int)> dfs = [&](int step){
            ...
            if(step == n+1 && prime[ans[1]+ans[n]]){
                // cout << "fuck   " << ans[1] << " " << ans[2] << ans[3] << " " << ans[4] << endl;
                for(int i = 1; i <= n; i++){
                    cout << ans[i] << " \n"[i == n];
                }
                return ;
            ...
            for(int i = 2; i <= n; i++){
                if(!a[i] && prime[i+ans[step-1]]){
                    ans[step] = i;
                    // cout << "ans[" << step << "] == " << ans[step] << endl;
                    a[i] = 1;
                    dfs(step+1);
                    a[i] = 0;
                }
