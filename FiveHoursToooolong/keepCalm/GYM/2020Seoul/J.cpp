...
int main() {
	...
	int n; cin >> n; Mat jz(n, vector<int> (n));
	for(int j = 0; j < n; j++) 
		for(int i = 0; i < n; i++)
			cin >> jz[i][j];
	if(det(jz) == 0) return cout << -1, 0;
	Mat iv = getInv(jz);
	cerr << iv.size() << ' ' << iv[0].size() << endl;
	for(int i = 0; i < n; i++) {
		Mat C(n, vector<int> (1));
		C[i][0] = 1;
		Mat ans = mul(iv, C);
		for(int j = 0; j < n; j++) {
			if(ans[j][0] % 2) cout << j+1 << ' ';
		}
		cout << endl;
	}
	return 0;
