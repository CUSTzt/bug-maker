...
int n , p;
pii line[maxn];
#define y1 askjhdalds
#define y2 akslhdjnlkja
#define x1 asdnjlad
#define x2 ahdslkdjn
int main()
{
    ...
    int tp = 1000;
    while(tp--){
        int p1 = 1ll * rnd(n) * rnd(n) % n + 1;
        int p2 = 1ll * rnd(n) * rnd(n) % n + 1;
        while(p1 == p2) p2 = 1ll * rnd(n) * rnd(n) % n + 1; 
        ll x1 = line[p2].fi - line[p1].fi, y1 = line[p2].se - line[p1].se;
        int cnt = 0;
        for(int i = 1; i <= n; i++){
            ll x2 = line[i].fi - line[p1].fi, y2 = line[i].se - line[p1].se;
            if(x2 * y1 == x1 * y2){
                cnt++;
            } 
        }
