...
void up(){
    int n;
    cin >> n;
    vi pre(n+10), suf(n+10), a(n+10);
    for(int i = 1; i <= n; i++)cin >> a[i];
    for(int i = 1; i <= n; i++){
        pre[i] = (a[i] >= pre[i-1] ? a[i] - pre[i-1] : linf / 2);
    }
    for(int i = n; i >= 1; i--){
        suf[i] = (a[i] >= suf[i+1] ? a[i] - suf[i+1] : linf);  
    }
    int fl = 1;
    for(int i = 1; i < n; i++){
        if(pre[i] == suf[i+1]){
            ...
            ll tp1 = a[i], tp2 = a[i+1];
            if(tp1 >= suf[i+2] and tp2 >= pre[i-1]){
                tp1-=suf[i+2], tp2 -= pre[i-1];
                ll num = tp1-tp2;
                if(!num){
                    fl = 0;
                    break;
