...
已经选择了 K 个数
最大值模完 K 的值为 l
dp[j][k][l][0] = max(dp[j - 1][k][l][0] , dp[j - 1][k - 1][l][0])
dp[j][k][(l + x) % K][1] = max(dp[j - 1][k - 1][l][0] , dp[j - 1][k - 1][l][1]) + a[i];
*/
int main() {
        ...
        for (int j = 1; j <= m; j++) {
            ...
            for (int l = 0; l < K; l++) {
                for (int k = 1; k <= m / 2; k++) {
                    ...
                    int he = -0x3f3f3f3f;
                    if (k)
                        he = max(dp[j - 1][k - 1][l][0],
                                 dp[j - 1][k - 1][l][1]) +
                             x;
                    int c = max(dp[j - 1][k][(l + x) % K][0],
                                dp[j - 1][k][(l + x) % K][1]);
                    dp[j][k][(l + x) % K][1] =
                        max(dp[j][k][(l + x) % K][1], max(he, c));
                }
            }
        }
        for (int h = 0; h <= m / 2; h++)
            for (int l = 0; l < K; l++)
                for (int L = 0; L < K; L++) {
                    ...
                    ans[i][(l + x) % K] =
                        max(max(y, ans[i - 1][(l + x) % K]), ans[i - 1][l] + x);
                }
    }
    res = max(res, ans[n][0]);
    cout << res;
