...
constexpr int maxn = 2000+10; // remember to calculate. if tle, check maxn first.
int n , m ;
int dp[maxn][maxn], X[maxn][maxn],Y[maxn][maxn], Z[maxn][maxn];
char c[maxn][maxn];
int32_t main()
{
    int h,w;
    ...
    for(int i = 1;i<=h;i++){
        for(int j = 1;j<=w;j++){
            if(c[i][j] == '.'){
                if(i == 1 && j == 1) continue;
                if(j > 1) X[i][j] = (X[i][j-1] + dp[i][j-1]) % mod;
                if(i > 1) Y[i][j] = (Y[i-1][j] + dp[i-1][j]) % mod;
                if(i > 1 && j > 1) Z[i][j] = (Z[i-1][j-1] + dp[i-1][j-1])% mod;
