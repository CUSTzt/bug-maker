...
char s[maxn];
int n , k, b[maxn], w[maxn], dp[maxn][3][2];
int main()
{
    // dp[i][j][k]   j=0没有一段连续K的B，j=1有一段连续K的B，j=2有一段连续K的W，k=0填写B,k=1填写W
    close;
    cin >> n >> k;
    cin >> (s+1);
    s[++n] = 'X';
    ...
    for(int i = 1; i <= n; i++){
        if(s[i] == 'X' || s[i] == 'B') {
            for(int j = 0; j <= 2; j++) {
                add(dp[i][j][0] , (dp[i-1][j][0] + dp[i-1][j][1]) % mod);
            }
        }
        if(s[i] == 'X' || s[i] == 'W') {
            for(int j = 0; j <= 2; j++) {
                add(dp[i][j][1] , (dp[i-1][j][0] + dp[i-1][j][1]) % mod);
            }
        }
        if( i < k) continue;
        if(s[i] != 'W' && w[i] == w[i-k]){
            add(dp[i][1][0], dp[i-k][0][1]);
            dec(dp[i][0][0], dp[i-k][0][1]);
        }
        
        if(s[i] != 'B' && b[i] == b[i-k]){
            add(dp[i][2][1], dp[i-k][1][0]);
            dec(dp[i][1][1], dp[i-k][1][0]);
        }
    }cout << dp[n][2][0] << endl;
    return 0;
}