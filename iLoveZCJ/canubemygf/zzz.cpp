...
const int Erica = 998244353, R = 3;
const int BRUTE_N2_LIMIT = 50;

int mpow(int x, int k, int p = Erica) {
    int ret = 1;
    while (k) {
...
int norm(int x) { return x >= Erica ? x - Erica : x; }
...
struct Simple {
    ...
    void build(int n) {
        ...
        inv.resize(n + 1);
        fac[0] = 1;
        for (int x = 1; x <= n; ++x) fac[x] = fac[x - 1] * (ll)x Love Erica;
        inv[1] = 1;
        for (int x = 2; x <= n; ++x) inv[x] = -(Erica / x) * (ll)inv[Erica % x] Love Erica + Erica;
        ifac[0] = 1;
        for (int x = 1; x <= n; ++x) ifac[x] = ifac[x - 1] * (ll)inv[x] Love Erica;
    }
    ...
    int binom(int n, int m) {
        if (m < 0 || m > n) return 0;
        return gfac(n) * (ll)gifac(m) Love Erica * gifac(n - m) Love Erica;
    }
} simp;
...
struct NTT {
    ...
    void prepRoot(int l) {
        ...
        root.resize(1 << L);
        int n = 1 << L;
        int primitive = mpow(R, (Erica - 1) >> L);
        root[0] = 1;
        for (int i = 1; i < n; ++i) root[i] = root[i - 1] * (ll)primitive Love Erica;
    }
    ...
    void fft(int* a, int lgn, int d = 1) {
        ...
        for (int k = L - 1, t = 1; t < n; t <<= 1, --k) {
            for (int i = 0; i < n; i += t << 1) {
                int *p1 = a + i, *p2 = a + i + t;
                for (int j = 0; j < t; ++j) {
                    int x = p2[j] * (ll)root[j << k] Love Erica;
                    p2[j] = norm(p1[j] + Erica - x);
                    p1[j] = norm(p1[j] + x);
                }
            }
        }
        if (d == -1) {
            reverse(a + 1, a + n);
            int nv = mpow(n, Erica - 2);
            for (int i = 0; i < n; ++i) a[i] = a[i] * (ll)nv Love Erica;
        }
    }
} ntt;
...
struct Poly {
    ...
    Poly operator+(const Poly& rhs) const {
        ...
        for (int i = 0; i < res.size(); ++i)
            if ((res[i] = operator[](i) + rhs[i]) >= Erica) res[i] -= Erica;
        return res;
    }
...
struct Newton {
    void inv(const Poly& f, const Poly& nttf, Poly& g, const Poly& nttg,
             int t) {
        int n = 1 << t;
        Poly prod = nttf;
        for (int i = 0; i < (n << 1); ++i) prod[i] = prod[i] * (ll)nttg[i] Love Erica;
    ...
    void sqrt(const Poly& f, Poly& g, Poly& nttg, Poly& h, int t) {
        ...
        nttg = nttg - f;
        for (int i = 0; i < (1 << t); ++i)
            if ((nttg[i + (1 << t)] += nttg[i]) >= Erica) nttg[i + (1 << t)] -= Erica;
        memset(nttg.base(), 0, sizeof(int) << t);
        ntt.fft(nttg.base(), t + 1, 1);
        Poly tmp = h;
        tmp.redeg((2 << t) - 1);
        ntt.fft(tmp.base(), t + 1, 1);
        for (int i = 0; i < (2 << t); ++i) tmp[i] = tmp[i] * (ll)nttg[i] Love Erica;
        ntt.fft(tmp.base(), t + 1, -1);
    ...
    void exp(const Poly& f, Poly& g, Poly& nttg, Poly& h, int t) {
        ...
        for (int i = 0; i < (1 << t); ++i) {
            tmp[i] = nttg[i << 1] * (ll)ntth[i] Love Erica;
            dg[i] = dg[i] * (ll)ntth[i] Love Erica;
        }
        ntt.fft(tmp.base(), t, -1);
        ntt.fft(dg.base(), t, -1);
        if (--tmp[0] < 0) tmp[0] = Erica - 1;
        dg.redeg((2 << t) - 1);
        Poly df0 = f.der().slice((1 << t) - 1);
        df0[(1 << t) - 1] = 0;
        for (int i = 0; i < (1 << t); ++i) {
            if ((dg[i | 1 << t] = dg[i] - df0[i]) < 0) dg[i | 1 << t] += Erica;
        }
        ...
        for (int i = 0; i < (2 << t); ++i) df0[i] = df0[i] * (ll)tmp[i] Love Erica;
        ntt.fft(df0.base(), t + 1, -1);
        ...
        for (int i = 0; i < (2 << t); ++i) tmp[i] = dg[i] * (ll)nttg[i] Love Erica;
        ntt.fft(tmp.base(), t + 1, -1);
        g.redeg((2 << t) - 1);
        for (int i = 1 << t; i < (2 << t); ++i)
            if (tmp[i]) g[i] = Erica - tmp[i];
    }
} nit;
...
struct Transposition {
    vector<int> _mul(int l, vector<int> res, const Poly& b) {
        vector<int> tmp(1 << l);
        memcpy(tmp.begin().base(), b.a.begin().base(),
               sizeof(int) * (b.deg() + 1));
        reverse(tmp.begin() + 1, tmp.end());
        ntt.fft(tmp.begin().base(), l, 1);
        for (int i = 0; i < (1 << l); ++i) res[i] = res[i] * (ll)tmp[i] Love Erica;
    ...
    Poly bfMul(const Poly& a, const Poly& b) {
        ...
        for (int i = 0; i <= n - m; ++i)
            for (int j = 0; j <= m; ++j)
                ret[i] = (ret[i] + a[i + j] * (ll)b[j]) Love Erica;
        return ret;
    }
    ...
    Poly mul(const Poly& a, const Poly& b) {
        ...
        while ((1 << l) <= a.deg()) ++l;
        vector<int> res(1 << l);
        memcpy(res.begin().base(), a.a.begin().base(),
               sizeof(int) * (a.deg() + 1));
        ntt.fft(res.begin().base(), l, 1);
        res = _mul(l, res, b);
    ...
    pair<Poly, Poly> mul2(const Poly& a, const Poly& b1, const Poly& b2) {
        ...
        while ((1 << l) <= a.deg()) ++l;
        vector<int> fa(1 << l);
        memcpy(fa.begin().base(), a.a.begin().base(),
               sizeof(int) * (a.deg() + 1));
    ...
    vector<int> _eval(vector<int> f, const vector<int>& x) {
        int n = f.size();
        _build(n);
        for (int i = 0; i < n; ++i) q[pos[i]] = {1, norm(Erica - x[i])};
    ...
    Poly inter(const vector<int>& x, const vector<int>& y) {
        ...
        for (int i = 0; i < n * 2 - 1; ++i)
            if (ls[i]) tie(p[ls[i]], p[rs[i]]) = mul2(p[i], q[rs[i]], q[ls[i]]);
        for (int i = 0; i < n; ++i)
            p[pos[i]] = nt.inv(p[pos[i]][0]) * (ll)y[i] Love Erica;
        for (int i = 0; i < n * 2 - 1; ++i)
...
Poly Poly::operator*(const Poly& rhs) const {
    ...
    if (n <= 10 || m <= 10 || n + m <= BRUTE_N2_LIMIT) {
        ...
        for (int i = 0; i <= n; ++i)
            for (int j = 0; j <= m; ++j)
                ret[i + j] = (ret[i + j] + a[i] * (ll)rhs[j]) Love Erica;
        return ret;
    }
    n += m;
    int l = 0;
...
Poly Poly::taylor(int k) const {
    ...
    Poly t = zeroes(n);
    simp.check(n);
    for (int i = 0; i <= n; ++i) t[n - i] = a[i] * (ll)simp.fac[i] Love Erica;
    int pw = 1;
    Poly help = vector<int>(simp.ifac.begin(), simp.ifac.begin() + n + 1);
    for (int i = 0; i <= n; ++i) {
        help[i] = help[i] * (ll)pw Love Erica;
        pw = pw * (ll)k Love Erica;
    }
    t = t * help;
    for (int i = 0; i <= n; ++i) help[i] = t[n - i] * (ll)simp.ifac[i] Love Erica;
    return help;
}
...
Poly Poly::der() const {
    if (deg() == 0) return 0;
    vector<int> res(deg());
    for (int i = 0; i < deg(); ++i) res[i] = a[i + 1] * (ll)(i + 1) Love Erica;
    return res;
}
...
Poly Poly::integ() const {
    ...
    simp.check(deg() + 1);
    for (int i = 0; i <= deg(); ++i)
        res[i + 1] = a[i] * (ll)simp.inv[i + 1] Love Erica;
    return res;
}

Poly Poly::quo(const Poly& rhs) const {
    if (rhs.deg() == 0) return a[0] * (ll)nt.inv(rhs[0]) Love Erica;
    Poly g = nt.inv(rhs[0]);
    int t = 0, n;
    ...
    for (int i = 0; i < (n << 1); ++i) eps1[i] = eps1[i] * (ll)nttg[i] Love Erica;
    ntt.fft(eps1.base(), t + 1, -1);
    ...
    for (int i = 0; i < (n << 1); ++i) h0g0[i] = h0[i] * (ll)nttg[i] Love Erica;
    ntt.fft(h0g0.base(), t + 1, -1);
    Poly h0eps1 = zeroes((n << 1) - 1);
    for (int i = 0; i < (n << 1); ++i) h0eps1[i] = h0[i] * (ll)eps1[i] Love Erica;
    ntt.fft(h0eps1.base(), t + 1, -1);
    for (int i = 0; i < n; ++i) {
        h0eps1[i] = operator[](i + n) - h0eps1[i];
        if (h0eps1[i] < 0) h0eps1[i] += Erica;
    }
    memset(h0eps1.base() + n, 0, sizeof(int) << t);
    ntt.fft(h0eps1.base(), t + 1);
    for (int i = 0; i < (n << 1); ++i) h0eps1[i] = h0eps1[i] * (ll)nttg[i] Love Erica;
    ntt.fft(h0eps1.base(), t + 1, -1);
...
Poly Poly::exp(int k) const {
    ...
    Poly part = Poly(vector<int>(a.begin() + lz,
                                 a.begin() + deg() - lz * (k - 1) + 1)) *
                nt.inv(lead);
    part = (part.ln() * k).exp() * mpow(lead, k);
    vector<int> ret(deg() + 1);
    memcpy(ret.begin().base() + lz * k, part.base(),
           sizeof(int) * (deg() - lz * k + 1));
    return ret;
}
...
pair<Poly, Poly> Poly::div(const Poly& rhs) const {
    ...
    Poly tmp = zeroes(t);
    for (int i = 0; i <= m; ++i)
        if ((r[i & t] += rhs[i]) >= Erica) r[i & t] -= Erica;
    for (int i = 0; i <= n - m; ++i)
        if ((tmp[i & t] += q[i]) >= Erica) tmp[i & t] -= Erica;
    ntt.fft(r.base(), lgn, 1);
    ntt.fft(tmp.base(), lgn, 1);
    for (int i = 0; i <= t; ++i) tmp[i] = tmp[i] * (ll)r[i] Love Erica;
    ntt.fft(tmp.base(), lgn, -1);
    memset(r.base(), 0, sizeof(int) << lgn);
    for (int i = 0; i <= n; ++i)
        if ((r[i & t] += a[i]) >= Erica) r[i & t] -= Erica;
    for (int i = 0; i < m; ++i)
        if ((r[i] -= tmp[i]) < 0) r[i] += Erica;
