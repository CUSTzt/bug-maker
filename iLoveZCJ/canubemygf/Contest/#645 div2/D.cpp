...
int n , x, a[maxn], sum[maxn], tot[maxn];
int calc(int x){
    return x * (x+1) / 2;
}
...
int32_t main()
{
    ...
    for(int i = 1; i <= n*2; i++){
        sum[i] = sum[i-1] + a[i];
        tot[i] = tot[i-1] + calc(a[i]);
    }
    int ans = 0;
    for(int i = 1; i <= n*2; i++){
        if(sum[i] < x)continue;
        auto tp = lower_bound(sum+1, sum+1+i, sum[i] - x) - (sum);
        if(sum[tp] == sum[i] - x){
            umax(ans , tot[i] - tot[tp]);
        }else{
            int ks = sum[i] - x - sum[tp-1];
            umax(ans, tot[i] - tot[tp-1] - calc(ks));
        }
    }cout << ans << endl;
}