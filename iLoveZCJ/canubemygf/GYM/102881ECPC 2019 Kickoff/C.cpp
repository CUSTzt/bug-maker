...
char s[maxn][maxn];
int a[maxn][maxn], turn[maxn][maxn];
...
int main()
{
    ...
    now = make_tuple(stx , sty, 0, -1);
    queue<tuple<int , int , int, int> > que;
    que.push(now);
    while(que.size()){
        auto [x, y , step , fx] = que.front();
        que.pop();
        for(int i = 0 ; i < 4 ; i++){
            ...
            if(turn[xx][yy] >= newstep){
                turn[xx][yy] = newstep;
                que.push(make_tuple(xx , yy , newstep , i));
            }
        }
    }
    cout << (turn[edx][edy] == INF ? -1 : turn[edx][edy] - 1) << endl;
    return 0;
}