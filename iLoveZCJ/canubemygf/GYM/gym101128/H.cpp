...
ll MAX = (1ll<<63)-1, MIN = 0;

ll to_longlong(string s) {
    ll res = 1, len = s.size(), ans = 0;
...
//    freopen("out.txt","w",stdout);
    ...
    std::vector<ll> ans;
    for(int n = 1; n <= 64; n++){
        for(int m = 0; n+m <= 64;m++){
            string s1="",s0="";
            for(int i = 0; i < n; i++)s1+='1'; for(int i = 0; i < m; i++)s0+='0';
            string kp = "";int fl = 1;
            while(1){
                ...
                if(kp.size() >=64)break;
                ll tp = to_longlong(kp);
                if(tp >= MIN && tp <= MAX)ans.eb(tp);
                fl = -fl;
            }
        }
    }
