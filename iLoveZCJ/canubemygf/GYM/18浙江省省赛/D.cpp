...
char s[maxn];
ll dp[maxn][maxn], v[maxn], sum[maxn],num[maxn], pos[maxn], n, l , r;
// others
void up(){
    cin >> n >> (s+1);
    for(int i = 1; i <= n; i++){
    ...
    for(int i = l ; i >= 1; i--){
        for(int j = n - (l - i); j >= pos[i]; j--){
            ll tp = v[pos[i]] * (sum[num[i]+j-pos[i]]-sum[num[i]]);
            dp[i][j] = dp[i+1][j+1] + tp;
            if(j < n - (l - i)) dp[i][j] = max(dp[i][j], dp[i][j+1]);
        }
    ...
    ll ans = 0;
    for(int j = 1; j <= n; j++)umax(ans , dp[1][j]);
    cout << ans << endl;
}
int main()
{
