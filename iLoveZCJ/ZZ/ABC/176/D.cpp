...
int chk(int x, int y){
    if(vis[x][y])return 0;
    if(x <= n && x >= 0 && y <= m && y >= 0 && s[x][y] == '.')return 1;
    return 0;
}
int main()
{
    ...
    q.push_back(make_tuple(stx, sty, 0));
    while(q.size()){
        auto [a, b , c] = q.front();
        q.pop_front();
        if(vis[a][b]) continue;
        ...
        dis[a][b] = c;
        for(int i = 0; i < 4; i++){
            int xx = a+dx[i], yy = dy[i] + b;
            if(chk(xx, yy))q.push_front({xx,yy,c});
        }
        for(int xx = a-2; xx <= a+2; xx++){
            for(int yy = b-2; yy <= b+2; yy++){
                if(chk(xx, yy)){
                    q.push_back({xx, yy , c+1});
                }
            }
        }
    }
