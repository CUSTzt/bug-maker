...
int cc(vector<int> a, int nn1, vector<int> b, int nn2, vector<int> c, int n3) {
    int res = linf;
    for (int i = 0; i < nn1; i++) {
...
int a[maxn], b[maxn], c[maxn], nn1, nn2, n3;
int FUCK(int *a, int *b, int *c, int nn1, int nn2, int n3) {
    int res = linf;
    FOR(i, 1, 1+nn1) {
...
void solve3() {
    ...
    sort(c + 1, c + 1 + n3);
    int zz = linf;
    zz=min(zz,FUCK(c,a,b,n3,nn1,nn2));
    zz=min(zz,FUCK(a,b,c,nn1,nn2,n3));
    zz=min(zz,FUCK(b,a,c,nn2,nn1,n3));
    cout << zz << endl;
}
int32_t main() {
    close;
