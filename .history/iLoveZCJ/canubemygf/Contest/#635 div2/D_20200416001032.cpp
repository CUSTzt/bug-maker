...
int cc(vector<int> a, int nn1, vector<int> b, int n2, vector<int> c, int n3) {
    int res = linf;
    for (int i = 0; i < nn1; i++) {
...
int a[maxn], b[maxn], c[maxn], nn1, n2, n3;
int FUCK(int *a, int *b, int *c, int nn1, int n2, int n3) {
    int res = linf;
    FOR(i, 1, 1+nn1) {
        int LE = lower_bound(b + 1, b + 1 + n2, a[i]) - b;
        int RI = lower_bound(c + 1, c + 1 + n3, a[i]) - c;
        int t1, t2 , t3;
        if (LE > n3 || c[LE] > a[i]) {
            if (LE > 1) LE--;
        }
        ...
        LE = lower_bound(c + 1, c + 1 + n3, a[i]) - c;
        RI = lower_bound(b + 1, b + 1 + n2, a[i]) - b;
...
void solve3() {
    ...
    sort(c + 1, c + 1 + n3);
    int zz = linf;
    zz=min(zz,FUCK(c,a,b,n3,nn1,n2));
    zz=min(zz,FUCK(a,b,c,nn1,n2,n3));
    zz=min(zz,FUCK(b,a,c,n2,nn1,n3));
    cout << zz << endl;
}
int32_t main() {
    close;
