...
int n,ans ,a[maxn], vis[maxn], cnt[maxn][220], pos[maxn], far[maxn], last[maxn],
    pre[maxn], st[maxn], f[maxn];
void solve() {
    cin >> n;
    ...
    for (int j = 2; j <= M; j++) pos[j] = cnt[0][j] = 0;
    for (int i = 1; i <= n; i++) {
        pre[i] = pos[a[i]];
        pos[a[i]] = i;
    }
    ...
    for (int i = 1; i <= n; i++) {
        ...
        if (i < f[i]) {
            ...
            for(int j =1; j <= M; j++) ma = max(ma, cnt[r][j] - cnt[l - 1][j]);
