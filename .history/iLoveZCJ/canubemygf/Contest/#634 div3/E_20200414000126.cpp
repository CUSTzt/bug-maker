...
int n, ans, a[maxn], vis[maxn], pre_pre_cnt[maxn][210], zz[maxn], far[maxn],
...
void solve() {
    ...
    for (int i = 1; i <= n; i++) {
        cin >> a[i];
        for (int j = 1; j <= M; j++) pre_pre_cnt[i][j] = pre_pre_cnt[i - 1][j];
        pre_pre_cnt[i][a[i]]++;
    }
    for (int j = 2; j <= M; j++) zz[j] = pre_pre_cnt[0][j] = 0;
    for (int i = 1; i <= n; i++) {
        pre[i] = zz[a[i]];
        zz[a[i]] = i;
    }
    ...
    for (int i = 1; i <= n; i++) {
        ...
        if (i < f[i]) {
            ...
            for (int j = 1; j <= M; j++)
                ma = max(ma, pre_cnt[r][j] - pre_cnt[l - 1][j]);
