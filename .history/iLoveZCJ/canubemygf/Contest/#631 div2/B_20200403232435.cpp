...
int main()
{
    ...
    CASET{
        ...
        /*for(int i = mx+1;i<=n-2*mx;i++){
            if(mp.count(i) == 0)fl = 1;
        }*
        for(int i = 1; i <= n-mx;i++){
            if(i <= mx){
                if(mp.count(i)!=2){fl=1;cout <<mp.count(i)<<"kk"<<i<<endl;}
            }else {
                if(mp.count(i)!=1){fl=1;cout << "pp"<<i << endl;}
            }
        }
        //cout << "fl == " << fl << endl;
        if(fl){
