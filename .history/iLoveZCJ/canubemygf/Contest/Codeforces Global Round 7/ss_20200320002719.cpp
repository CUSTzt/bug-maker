...
void manacher(int n) {
    ...
    for (int i = 3; i < m; i++) {
        ...
        if (i + len[i] - 1 >= n * 2 + 1 || i - len[i] + 1 <= 3) {
            if (len[i] > re) {re = len;ANS1 = i;}
        }
    }
    for (int  ANS1 - re + 1;  ANS1 + re ; i++) {
...
int main()
{
    CASET{
        ...
        int lenb = strlen(bb + 1);
         ANS1 = 0;
        whi(ANS1 + 1 <= lenb / 2 &&[ANS1 + 1] == bb[len ANS ANS1++;
        int cnt = 0;
        for (int i = 1; i ANS1; i++) printf("%c", bb[i]);
        for (int  ANS1 + 1; i <= len ANS1; i++) aa[++cnt] = bb[i];
        manacher(cnt);
        for (int  ANS1; i >= 1; i--) printf("%c", bb[i]);
        cout<< endl;
    }
