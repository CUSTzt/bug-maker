...
struct NumberTheory {
  ...
  int sqrt(int x, int p = P) {
    ...
    while (k) {
      if (k & 1)
        res = pii((res.first * (ll)a.first + res.second * (ll)a.second % p * v) % p, (res.first * (ll)a.second + res.second * (ll)a.first) % p);
      if (k >>= 1)
        a = pii((a.first * (ll)a.first + a.second * (ll)a.second % p * v) % p, (a.first * (ll)a.second << 1) % p);
    }
