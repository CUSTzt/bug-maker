...
struct complex {
    ...
    friend inline complex operator*(D x, complex cp) {
...
void mtt(int deg, int *lhs, int *rhs, int *ret) {
    ...
    fft(len, H, true);
    for (int i = 0; i < len; i++)
        ret[i] = (((int64(round(E[i].real)) % MOD) << 30) % MOD +
                  ((int64(round(F[i].real)) % MOD) << 15) % MOD +
                  ((int64(round(G[i].real)) % MOD) << 15) % MOD +
                  int64(round(H[i].real)) % MOD) %
                 MOD;
}
