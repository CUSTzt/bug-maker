...
int main()
{
    close;
    cin >> n >> m;
    for(int i = 1; i <= n; i++) cin >> a[i], v1.push_back({a[i], i});
    for(int i = 1; i <= m; i++) cin >> b[i], v2.push_back({b[i], i});
    for(int i = 0; i < n; i++) cur.set(i);
    for(int i = 0; i < n - m + 1; i++) all.set(i);
    sort(ALL(v1)), sort(ALL(v2));
    int now = 0;
    for(int i = 0; i < m; i++){
        while(now < n && v1[now].fi < v2[i].fi){
            cur.reset(v1[now].se - 1);
            now++;
        }
        all = all & (cur >> (v2[i].se - 1));
    }
    cout << all.count();
}