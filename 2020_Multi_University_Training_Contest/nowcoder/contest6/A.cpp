...
int p[maxn], inv[maxn], ans[maxn], f[maxn];
void pre(){
    int tmp = (mod+1)/2;
    int iv = get_inv(tmp , mod);
    inv[1] = iv, p[1] = 2, p[0] = 1;
    for(int i = 2; i < 10;i++){
        p[i] = p[i-1] * 2ll % mod;
        inv[i] = 1ll * inv[i-1] * iv % mod;
        cout << "p == " << p[i] << "   iv == " << inv[i] << endl;
        f[i] = f[i-1] ^(1ll*(p[i]-1)*inv[i] % mod);
        ans[i] = ans[i-1] ^ f[i];
    }
}
int main()
