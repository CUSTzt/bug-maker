...
int main()
{
    while(cin >> X >> Y){
        ...
        memset(mn, INF, sizeof mx);
        tot = 0;
        for(int i = 0; i < n; i++){
            cin >> point[i].fi >> point[i].se;
            umax(mx[point[i].fi],point[i].se);
            umin(mn[point[i].fi],point[i].se);
        }
        vi mid;
        for(int i = 0; i < 100005;i++){
            if(mx[i] != -INF && mn[i] != INF){
                mid.eb(mx[i]);mid.eb(mn[i]);
            }
        }
        sort(ALL(mid));
        int sz = SZ(mid);
        ...
        for(int i = 0; i < 100005; i++){
            if(mx[i] != -INF && mn[i] != INF){
                ans += (1ll)*mx[i] - mn[i] + abs(pp - mx[i]) + abs(pp - mn[i]);
            }
        }
        cout << ans+X-1 << endl;
    }
