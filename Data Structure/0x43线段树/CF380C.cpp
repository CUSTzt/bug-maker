...
// l~r 中合法长度 为r-l+1-未匹配的'('-未匹配的')'
char s[maxn];
int tl[maxn<<3], tr[maxn<<3];//tl未未匹配的左括号数，tr为未匹配的右括号数
int n , m;
#define mid ((l+r)>>1)
#define lson (o<<1)
#define rson (o<<1|1)
...
pii ask(int o , int l , int r , int L , int R){
    // printf("%d %d %d %d %d\n",o, l , r , L , R);
    if(L <= l && r <= R) return {tl[o], tr[o]};
    if(R <= mid) return ask(lson, l , mid , L , R);
    if(L >  mid) return ask(rson , mid+1, r , L , R);
    pii ls = ask(lson, l , mid , L , R), rs = ask(rson, mid+1, r , L, R), tp;
    tp.fi = ls.fi + rs.fi - min(ls.fi, rs.se);
    tp.se = ls.se+rs.se-min(ls.fi,rs.se);
    return tp;
}
int32_t main()
{
    ...
    while (m--) {
        int x, y;
        cin >> x >> y ;
        pii ans = ask(1,1,n,x,y);
        cout << y-x+1-ans.fi-ans.se << endl;
    }
