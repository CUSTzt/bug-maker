...
int fib[maxn], n , g[maxn], ans, ok[maxn], a[maxn];
int32_t main()
{
    close;
    fib[1] = 1; g[1] = 1; 
    for(int i = 2; i < maxn; i++) {fib[i] = (fib[i-1] + fib[i-2]) % mod; g[i] = fib[i];}
    ans = 1;
    cin >> n;
    for(int i = 1;  i <= n; i++){
        cin >> a[i]; ok[a[i]] = 1;
    ...
    for(int i = 1; i < maxn; i++){
        for(int j = 1; j * i < maxn; j++){
            if(ok[i*j]) {ans = ans * g[i] % mod; break;}
        }
    }
    cout << ans << endl;
}