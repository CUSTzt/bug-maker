...
int main()
{
    ...
    // cin >> w >> m;
    read(w), read(m);
    if(w <= 3){cout << "YES" << endl;return 0;}
    // cout << "m === " << m << endl;
    function<void (ll , ll )> dfs = [&](ll now , ll tot){
        // cout << "now == " << now << "  tot == " << tot << "   " << (tot==m) <<  endl;
        // write(now);cout << "   "; write(tot); cout << endl;
        if(tot == m){
            cout << "YES" << endl;
            exit(0);
        }
