// n个节点(1<=n<=30000)的节点带权值的树,q次询问(0<=q<=200000)
//1.单点修改
//2.查询u到v的路径上的最大权值；
//3.查询u到v的路径上的权值之和。
#include <bits/stdc++.h>
...
struct segtree {
    ...
    int query_max(int l, int r) { return query1(1, l, r); }
    int query_sum(int l, int r) { return query2(1, l, r); }
} tree;
vector<int> G[MAXN];
...
void update(int k, int x) { tree.update(spos[k], x); }
