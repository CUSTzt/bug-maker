...
using namespace std;
const int u = 50010;
int ver[2 * u], edge[2 * u], next[2 * u], head[u], a[u], b[u], g[u], v[u],
    fa[u][16], sh[u], on[u];
...
void add(int ycat_l, int ycat_h, int ycat_v) {
    ver[++tot] = ycat_h, edge[tot] = ycat_v, next[tot] = head[ycat_l],
    head[ycat_l] = tot;
}
...
bool solve(long long T) {
    ...
    for (i = 1; i <= m; i++) {
        for (g[i] = a[i], d[i] = 0, j = 15; j >= 0; j--)
            if (fa[g[i]][j] && d[i] + dis[g[i]][j] <= T)
                d[i] += dis[g[i]][j], g[i] = fa[g[i]][j];
        on[g[i]] = 1;
        if (j = sh[g[i]]) {
            arv[j].push_back(T - d[i]);
            if (arv[j].size() > 1 && T - d[i] > arv[j][arv[j].size() - 2])
                swap(arv[j][arv[j].size() - 1], arv[j][arv[j].size() - 2]);
        }
    }
    for (i = 1, p = q = 0; i <= t; i++) {
        if (!dfs(ver[b[i]]))
            if (arv[i].size() && arv[i][arv[i].size() - 1] < edge[b[i]] * 2)
                arv[i].pop_back();
            else
                f[++q] = edge[b[i]];
