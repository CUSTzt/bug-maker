...
int main()
{
    ...
    ll INV = get_inv(3);
    for(int i = 1; i <= n; i++){
        s[i] == '?' ? cnt++,pw=pw*3ll%mod:1;
    }
    for(int i = 1; i <= n; i++){
        for(int j = 1; j <= 3; j++){
            dp[i][j] = dp[i-1][j];
        }
        if(s[i] == 'a') add(dp[i][1], pw);
        else if(s[i] == 'b') add(dp[i][2], dp[i-1][1]);
        else if(s[i] == 'c') add(dp[i][3], dp[i-1][2]);
        else {
            add(dp[i][1], 1ll * pw * INV % mod);
            add(dp[i][2], dp[i-1][1] * INV % mod);
            add(dp[i][3], dp[i-1][2] * INV % mod);
        }
    }
    cout << dp[n][3] << endl;
    return 0;
