...
{
    int u , v, w;
    friend int operator < (const node &a, const node &b){return a.w < b.w;}
    /* data */
}a[maxn];
int n , m, c[maxn], ans[maxn],sz[maxn];
int fa[maxn];
...
int32_t main()
{
    ...
    sort(a+1, a+n);
    for(int i = 1; i < n; i++){
        int faa=find(a[i].u), fb=find(a[i].v);
        c[a[i].w] += sz[faa]*sz[fb];
        fa[faa]=fb;
        sz[fb]+=sz[faa];
    }
